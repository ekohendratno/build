...
class update_system {
	...
	function cheking_updates(){
		...
		$server = esc_sql( $this->url_server . '/?api=2&content=release' );			
		//$this->server = simplexml_load_string( get_content($server) );	
		$this->server = try_xml( $server );
...
<project>	
	<version>
		...
		<code>(0-9)</code>
		<release>
			<name>(alpa|beta|rc|stable)</name>
			<code>(0-9)</code>
		</release>
	</version>
	<package>(0|1)</package> //update using package available or no available
	<date>(Y-m-d H:i:s)</date>
	<url>api.server/latest-(alpa|beta|rc|stable).zip</url>	
</project>
*/
