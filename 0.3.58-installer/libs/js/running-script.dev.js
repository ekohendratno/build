...
*/
	...
	function event_scroll(id,i){
		...
		$.event.add(window, "scroll", function() {
			...
			$('.shadow-inside-top').css('z-index',((p)>start) ? i : '');
			$(".nav").mouseover( function(){
				$('.shadow-inside-top').css('position',((p)>start) ? 'fixed' : 'absolute');
				$('.shadow-inside-top').css('z-index',((p)>start) ? i : '');
			});
			$(".nav-fix-sub").mouseout(function() {
				$('.shadow-inside-top').css('position','fixed');
			});
	...
	event_scroll('.nav.nav-fix',2);
	event_scroll('.nav.nav-fix-sub',1);
