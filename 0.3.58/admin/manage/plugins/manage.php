...
?>
	<tr class="isi" <?php echo $warna;?>>
	  <td style="text-align:left; vertical-align:middle; padding-left:5px"><input type="checkbox" name="plugin_path[]" value="<?php echo $key?>" id="checkbox_id_<?php echo $checkbox_id;?>" /></td>
	  ...
	  $style_button = 'middle';
	  echo '<a href="?admin&sys=plugins&go=setting&plugin_name='.$name.'" title="Setting this plugins" class="button button2 left">Setting</a>';
	  }
	  
	  if( get_plugins( $key ) == 1 ){
	  echo '<a href="?admin&sys=plugins&act=pub&pub=no&plugin_name='.$key.'" title="Deactive this plugin" class="button button2 '.$style_button.'">Deactive</a>';
	  }else{
	  echo '<a href="?admin&sys=plugins&act=pub&pub=yes&plugin_name='.$key.'" title="Activate this plugin" class="button button2 '.$style_button.'">Activate</a>';
	  }
	  
	  $key2 = str_replace( $name .'/', '' , $key );
	  
	  echo '<a href="?admin=single&sys=plugins&go=edit&plugin_name='.$name.'&file=/'.$key2.'" title="Open this file in the Plugin Editor" class="button button2 middle">Edit File</a><a href="?admin&sys=plugins&act=del&plugin_name='.$name.'" title="Delete this plugin" onclick="return confirm(\'Are You sure delete this plugins?\')" class="button button2 right">Delete</a>';
	  ?>      
      </td>
...
add_templates_manage( $content, 'Plugins Manager', $header_menu, null, $form );
...
$header_menu = '<a href="?admin&sys=plugins" class="button"><span class="icon_head back">&laquo; Back</span></a>';
add_templates_manage( $content, 'Setting Manager', $header_menu );
