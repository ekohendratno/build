...
$sql = $db->query( "SELECT * FROM `$db->post` $add_query ORDER BY id DESC LIMIT $posisi,$batas");
...
$paging->set('urlscheme','?admin&apps=post&p=%page%');
$paging->set('perpage',$batas);
$paging->set('page',$p);
...
if( $sfl == 'user' ){ 
$header_menu.= '<option value="?admin&apps=post'.$type.'&showforlevel=admin">Admin</option>
<option value="?admin&apps=post'.$type.'&showforlevel=user" selected="selected">User</option>';
}else{
$header_menu.= '<option value="?admin&apps=post'.$type.'&showforlevel=admin" selected="selected">Admin</option>
<option value="?admin&apps=post'.$type.'&showforlevel=user">User</option>';
}
$header_menu.= '</select></div>';
$header_menu.= '<a href="?admin=single&apps=post&go=add&type=post" class="button button3 left">+ Post</a><a href="?admin=single&apps=post&go=add&type=page" class="button button3 middle ">+ Page</a><a href="?admin&apps=post&go=addcat" class="button button3 right">+ Topik</a> <a href="?admin&apps=post&go=setting" class="button button3 black" style="margin-left:2px;">Setting</a>';

add_templates_manage( $content, 'Posting Manager', $header_menu );
...
$header_menu.= '<select name="type" style="margin-left:2px;"><option value="">-- Pilih Type --</option><option value="post" '.$select_post.'>Post</option><option value="page" '.$select_page.'>Page</option></select>';
