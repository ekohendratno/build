...
{ 
	...
	$sample			= filter_txt( $_POST['sample'] );
	...
	or !empty($sex))
	{
		...
		$data_sample = abs_path . "libs/installer/sample/data.sql";
		...
		if( file_exists('_config.php') ) 
		{
			...
			if( $sample ){
				...
				$replace_sample = array(
					"_installer_prefix_" => DB_PRE,
					"_installer_author_" => "$username"			
				);
				
				$go_data_sample = dump( $data_sample, $replace_sample );	
		...
		if( $go_data )
		{
			...
			if( $go_data_sample ){
				//move images to dir content/uploads<br>
				$root 	= abs_path;
				$tmp 	= "$root/libs/installer/sample/images";
				$move 	= "$root/content/uploads/post";
...
<table width="100%" border="0" cellpadding="2">
  ...
  <tr>
    <td>Sample</td>
    <td><strong>:</strong></td>
    <td><label><input name="sample" type="checkbox" value="1">Ikutkan contoh sample postingan & topik</label></td>
  </tr>
  <tr>
    <td colspan="3">&nbsp;</td>
  </tr>
