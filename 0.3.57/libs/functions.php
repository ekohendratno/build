...
function get_option_array_widget(){
	...
	$arrayx["sidebar-1"]= array("archives-1");
	$arrayx["sidebar-2"]= array("archives-1"); 
	$arrayx["sidebar-3"]= array("pages-1");
	$arrayx["sidebar-4"]= array("meta-1"); 
...
<li class="page_item page-item-31"><a href="" title="Page Templates">Page Templates</a></li>
<li class="page_item page-item-43"><a href="" title="Theme Options">Theme Options</a></li>';
...
function list_categories( $args = '' ) {
	...
	$order = "AND public = '1' ORDER BY $orderby $order  LIMIT $no_categories";

	global $db;
	
	$query_request = $db->select( 'post_topic', array( 'status' => 1, 'public' => 1), $order );
...
function get_archives($args = '') {
	...
	if ( 'monthly' == $type ) {
		...
		while ( $arcresult = $db->fetch_obj($arcresults) ) {
			...
			$url = do_links('post', array('view' => 'archive', 'id' => $id, 'title' => 'archive'));
...
function tag_cloud( $args = '' ) {
	...
	if( $args['id'] != '' )
		$add_query.= 'AND id='.$args['id'].' ';
		
	if( $args['status'] = '' )
		$add_query.= "AND status='1' ";
	
	$query = "SELECT * FROM $db->post WHERE type='post' $add_query";
	
	global $db;
	
	$tagresults = $db->query( $query );
...
function generate_tag_cloud( $tags, $args = '' ) {	
	...
	if ($totalTags > 0) {
		...
		foreach($jumlah_tag as $key=>$val) {
			...
			$datas  = array('view'=>'tag','id'=>urlencode($key));
...
function the_tags( $sep = ', ' ) {
	$id = get_query_var('id');
	echo tag_cloud( array('id' => $id,'echo' => false,'sep' => $sep) );
}
/**
...
function save_comment_data($r){
	...
	if( get_comment_login() && $login->login_level('admin') ) $approved = 1;
...
function doing_feed(){
		...
		//if( empty($feed_item[error]) ):
			...
			foreach($feed_item['feed_content'] as $item):
				if( $i <= $display->{'limit'} ){
					...
					if( $display->{'desc'} == 1 )
					$feed_content[$i]['desc'] = limittxt($item['description'],120);
	...
	$rssfeed = json_encode( $_temp );
}
add_action('dashboard_init','doing_feed');
...
<div id="dialog_<?php echo $dp_id?>"  class="redactor_modal" style="width: <?php echo $width?>px; height: <?php echo $height?>;display: none; ">
    ...
    <div class="redactor_content_border" style="padding:5px; background:#f8f8f8">
    <?php echo $dp_footer;?>
    <div style="clear:both"></div>
    </div>
    <?php endif;?>
