...
while( $data_post = $db->fetch_array($query_post) ){
	...
	'content' 	=> limittxt(strip_tags($data_post['content']),250),
...
$data_archive 	= array();
$query_post		= $db->select('post', array('type' => 'post', 'post_topic' => $id, 'approved' => 1, 'status' => 1) );
...
$sqlPost	= $db->select( "post", array('id' => $id, 'type' => 'post', 'approved' => 1, 'status' => 1) );
