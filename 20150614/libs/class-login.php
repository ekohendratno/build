...
class login{
	...
	function activation($codeaktivasi){
		...
		if( $this->_activation($key) )
			$this->text_message = array('type'=>'message','text'=>'Data telah dikirim ke email anda');
		else 
			$this->text_message = array('type'=>'error','text'=>'Gagal mengirim data ke email');
	}
	...
	function change_password( $data ){	
		...
		if( is_array($msg) ) 
		{
			foreach($msg as $val) $this->text_message = array('type'=>'error','text'=>$val);
		}
		
		if(empty($msg)){
			$update = $this->_change_password(compact('user_pass'),compact('user_login'));
			if( $update ) $this->text_message = array('type'=>'success','text'=>'The Password success to change');
		}
	}	
	/*
	 * lost password
	...
	function lost_password($email){
		...
		if( is_array($msg) )	{
			foreach($msg as $val)
				$this->text_message = array('type'=>'error','text'=>$val);
		}else{
				
		if( $this->_lost_password( $user_email ) )
			$this->text_message = array('type'=>'message','text'=>'Link aktivasi telah dikirim ke email anda, silahkan melakukan aktivasi');
		else 
			$this->text_message = array('type'=>'error','text'=>'Gagal mengirim aktivasi ke email');
	...
	function _in($data){
		...
		if( !empty( $msg ) )
			$this->text_message = array('type'=>'error','text'=>$msg);
		...
		if( $redirect ){
			$this->text_message = array('type'=>'success','text'=>'Redirect...');
		...
		if( $msg != null && is_array($msg))	{
			...
			if( $db->insert( 'users', $data) ){
				...
				if( $this->message_activation($user_data) )
					$this->text_message = array('type'=>'success','text'=>'Anda berhasil menambahkan akun, cek email kamu untuk verifikasi');
			}
		}
	}
	/*
	...
	function _update_user( $userdata ){
		...
		if( $msg == null && $db->update( 'users', $data, compact( 'ID' ) ) ):
			$this->text_message = array('type'=>'success','text'=>'Berhasil memperbaharui akun');
	...
	function login_out( $return = false ){	
		...
		if( $return ){
			...
			if( isset( $_SESSION['username'] ) ){
				if( $this->_clear_log() )
				$this->text_message = array('type'=>'success','text'=>'Anda telah keluar dari website');
			}
	...
	function message_reg($data){	
		...
		if( mail_send($email, $head, $send) )
			$this->text_message = array('type'=>'message','text'=>'Akun dan Sandi telah dikirim ke email anda, silahkan login');
		else 
			$this->text_message = array('type'=>'error','text'=>'Gagal mengirim data akun ke email.');
