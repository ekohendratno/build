jQuery(function($) {
	...
	var fixSortable = function() {
		...
		//this is fix for ie
		$('#dragbox_easymn').NestedSortableDestroy();
		$('#dragbox_easymn').NestedSortable({
			accept: 'sortable_easymn',
			...
			buttons: {
				'Save': function() {
					...
					if (group_title == '') {
						...
						$.ajax({
							type: 'POST',
							url: site_url('?request&load=libs/ajax/menu_group.php&aksi=add'),
	...
	$('#form-menu').submit(function() {
		...
		$.ajax({
			...
			error: function() {
				$('#btn-save-menu').attr('disabled', false);
				gbox.show({
					content: '<h2>Error</h2><div class="padding">Save menu error. Please try again.</div>',
					autohide: 1000
				});
			},
			success: function(data) {
				gbox.show({
					content: '<h2>Success</h2><div class="padding">Menu position has been saved</div>',
					autohide: 1000
				});
			}
		});
	...
	$('#edit-group').click(function() {
		...
		inputgroup.focus().select().keydown(function(e) {
			if (e.which == 13) {
				...
				$.ajax({
					type: 'POST',
					url: site_url('?request&load=libs/ajax/menu_group.php&aksi=edit'),
	...
	$('#delete-group').click(function() {
		...
		gbox.show({
			...
			buttons: {
				'Yes': function() {
					$.post(site_url('?request&load=libs/ajax/menu_group.php&aksi=delete'), param, function(data) {
						if (data.success) {
