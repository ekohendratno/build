...
if(!function_exists('set_general')){
	function set_general($data){
		...
		if( empty($timeout) ) $msg[] ='<strong>ERROR</strong>: The timeout is empty.';
		if( empty($author) ) $msg[] ='<strong>ERROR</strong>: The author is empty.';
		if( empty($account_registration) ) $msg[] ='<strong>ERROR</strong>: The account registration is empty.';
		...
		if($msg) foreach($msg as $error) _e('<div id="error">'.$error.'</div>');
		else{
...
if(!function_exists('save_options')){
	function save_options($data){
		...
		foreach ( (array) array_keys( $data ) as $name ) {
			if( !checked_option( $name ) ) $run[] = add_option( $name, $data[$name] );
			else $run[] = set_option( $name, $data[$name] );
		}
		return $run;
