...
$sql_query = $db->query("SELECT * FROM iw_users $add_query ORDER BY user_registered DESC LIMIT $limit");
$warna = '';
...
<img alt="" src="<?php _e( $avatar_img_profile )?>" class="avatar" height="50" width="50">
<div class="dashboard-comment-wrap">
<span class="name"><?php _e( uc_first($row['user_login']) )?></span><br />
Negara: <span class="country"><em><?php if(empty($row['user_country'])) _e('unknow'); else _e($class_country->country_name($row['user_country']));?></em></span>
,Provinsi: <span class="prov"><em><?php if(empty($row['user_province'])) _e('unknow'); else _e($row['user_province']);?></em></span><br /> 
Tanggal: <span class="date"><em><?php _e( datetimes($row['user_registered']) )?></em></span>	
			
<p class="row-actions">
<a href="?admin&sys=users&go=edit&to=data&id=<?php _e( $row['ID'] )?>" class="button button2 left">Edit Data</a>
<a href="?admin&sys=users&go=edit&to=pass&id=<?php _e( $row['ID'] )?>" class="button button2 middle">Edit Password</a>
<a href="?admin&sys=users&act=del&id=<?php _e( $row['ID'] )?>"  onclick="return confirm('Are You sure delete this users?')" class="button button2 right">Trash</a>
</p>
...
?>
  <table width="100%" border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td align="left"><label>User Name <strong>:</strong></label><br /> 
        <input type="text" name="username" value="<?php _e($r['user_login'])?>" style="width:550px"></td>
      </tr>
    <tr>
      <td align="left"><label>E-Mail <strong>:</strong></label><br /> 
        <input type="text" name="email" value="<?php _e($r['user_email'])?>" style="width:550px"></td>
      </tr>
    <tr>
      <td align="left"><label>Author Name <strong>:</strong></label><br /> 
        <input type="text" name="author" value="<?php _e($r['user_author'])?>" style="width:550px"></td>
      </tr>
    <tr>
      <td align="left"><label>Seorang <strong>:</strong></label><br /> 
        <select name="sex">
    ...
    <tr>
      ...
      <input type="text" name="province" value="<?php _e($r['user_province'])?>" style="width:550px;"></td>
      </tr>
  </table>
<?php
$header_menu = '<a href="?admin&sys=users" class="button button3 left"><span class="icon_head back">&laquo; Back</span></a>';
