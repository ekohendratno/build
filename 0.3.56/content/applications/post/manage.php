...
global $iw, $db, $login, $class_country, $widget;
...
$sql = $db->query( "SELECT * FROM `".$iw->pre."post` $add_query ORDER BY id DESC LIMIT $limit");
?>
...
<tr <?php _e($warna)?> class="isi">
	...
	<td valign="top"><div align="center"><?php _e($approved)?></div></td>
	<td valign="top"><div align="center"><?php _e($status)?></div></td>
    <td valign="top">
    <div class="action">
<a href="?com=post&view=item&id=<?php _e($id)?>" class="view" title="view" target="_blank">view</a>
<a href="?admin=single&apps=post&go=edit<?php _e($type)?>&id=<?php echo $id . $from;?>" class="edit" title="edit">edit</a>
...
?>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td colspan="2"><label for="isi">Isi:</label><?php iw_editor('','idEditor', array('editor_name' => 'isi','editor_style' => 'width:550px; height:250px;') );?></td>
    </tr>
    <tr>
...
?>
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td style="width:70%"><label for="title">Judul: <span class="required">*</span></label><br /><input type="text" id="title" name="title" value="<?php _e($row['title'])?>" placeholder="Judul Posting" required="required" style="width:99%;" /></td>
      <td align="right">
      <?php if( $type != 'page' ):?>
    ...
    <tr>
      <td colspan="2"><label for="isi">Isi:</label><?php iw_editor( $row['content'] ,'idEditor', array('editor_name' => 'isi','editor_style' => 'width:550px; height:250px;') );?></td>
    </tr>
<?php if( $type != 'page' ):?>
    <tr>
      <td colspan="2"><label for="tags">Tags: *(</label><br /><input id="tags" type="text" name="tags" style="width:98%;" value="<?php _e($row['tags'])?>"/></td>
    </tr>
    <tr>
      <td><label for="thumb">Ganti Thumbnail:</label><br /><div id="file">Chose file</div><input id="thumb" type="file" name="thumb"></td>
      <td align="right"><label for="thumb_desc">Keterangan Gambar *(:</label><input id="thumb_desc" type="text" name="thumb_desc" value="<?php _e($row['thumb_desc'])?>" style="width:90%;"/></td>
      </tr>
<?php endif;?>
    <tr>
      <td></td>
...
<?php
$limit		= 10;
$sql		= $db->query( "SELECT * FROM `iw_post_topic` ORDER BY id DESC LIMIT $limit");
$warna 		= '';
while($data = $db->fetch_array($sql)) {
$id 		= $data['id'];
$title 		= $data['topic'];
$q2			= $db->query("SELECT COUNT(*) AS jumNews FROM `iw_post` WHERE post_topic='$id'");
...
if( isset($_POST['submitDelete']) ) {
	$commentar_id = $_POST['commentar_id'];
	
	if(is_array($commentar_id)){	
...
<strong>Title : <?php _e($row_post->title)?></strong>
</div>
<?php
if( isset($_POST['submit']) ){
...
<div class="comment-img" style="margin-left:5px; margin-top:5px; margin-right:10px"><img src="<?php _e(get_gravatar($data['email']));?>" /></div>
...
<div class="comment-img comment-reply-img" style="margin-left:15px;"><img src="<?php _e(get_gravatar( get_option('admin_email') ));?>" /></div>
...
<input type="checkbox" name="commentar_id[]" value="<?php _e($data2['comment_id'])?>" id="checkbox_id_<?php echo $checkbox_id;?>" />
...
?>
	<tr class="isi" <?php echo $warna;?>>
	  <td style="text-align:left; vertical-align:middle; padding-left:5px"><input type="checkbox" name="commentar_id[]" value="<?php _e($row_comment->comment_id)?>" id="checkbox_id_<?php echo $checkbox_id;?>" /></td>
	  <td>
      <div style="float:left; width:42px; height:100%; margin:2px; margin-right:5px;">
      ...
      <?php _e($row_comment->author)?><br />
      <a href="?admin&apps=post&go=comment&act=view&id=<?php _e($row_comment->comment_id)?>" class="button button4 green" style="margin-top:3px;">Lihat</a><a href="?admin&apps=post&go=comment&act=view&reply=1&id=<?php _e($row_comment->comment_id)?>" class="button button4 blue" style="margin-top:3px;">Balas</a>
	  </div>
      <div style="clear:both"></div>
      </td>
	  <td>
      <strong><?php _e($row_post->title)?></strong>
      <p><?php _e($row_comment->comment)?></p>
      <div style="clear:both"></div>
      <a href="mailto:<?php _e($row_comment->email)?>" title="Visit autdor homepage">Send a Mail</a> &bull; <?php _e( time_stamp($row_comment->time) )?> 
      <div style="clear:both"></div>
      </td>
    </tr>
<?php
