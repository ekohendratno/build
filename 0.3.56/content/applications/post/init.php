...
function get_widget(){
	...
	if( $_GET['go'] != 'comment' )
	$gadget[] = array('title' => 'Menunggu persetujuan','desc' => article_approved() );	
	
	$gadget[] = array('title' => 'Komentar Terbaru','desc' => article_comment() );	
...
function article_approved(){
	...
	while( $row_post = $db->fetch_obj( $sql_post ) ){
		...
		$post .= '<li'.$warna.'><img src="'.get_gravatar($field->user_email).'" style="float:left; width:40px; height:40px; margin-right:5px;">'.$row_post->title;
		$post .= '<div style="clear:both; padding-bottom:5px;"></div>';
		$post .= '<div style="float:left; width:60%;"><a href="?admin&apps=post&type=post&act=approv&pub=yes&id='.$row_post->id.'" class="button button2 on left">Setuju</a><a href="?admin&apps=post&act=del&id='.$row_post->id.'" class="button button2 right" onclick="return confirm(\'Are You sure delete this post?\')">Hapus</a></div>';
...
function article_comment(){
	...
	$sql_comment = $db->query( "SELECT * FROM ".$iw->pre."post_comment $add_query ORDER BY date DESC LIMIT 30" );
	...
	while( $row_comment = $db->fetch_obj( $sql_comment ) ){
		...
		$comment .= '<li'.$warna.'><img src="'.get_gravatar($row_comment->email).'" style="float:left; width:40px; height:40px; margin-right:5px;">'.$row_comment->author. ', ' .time_stamp($row_comment->time) . '<br>';
		...
		if( $row_comment->comment_parent > 0 ){
			...
			$comment_content = '<a href="?admin&apps=post&go=comment&act=view&reply=1&id='.$comment_id.'" class="button button4 on middle blue">Jawab</a>';	
		}
		
		$sql_post = $db->select( 'post' , array('id' => $row_comment->post_id) );
		$row_post = $db->fetch_obj( $sql_post );
		
		$comment .= '<a href="?admin&apps=post&go=comment&act=view'.$reply.'&id='.$comment_id.'" class="button button4 on left '.$add_color.'">'.$add_action.'</a>';	
		$comment .= $comment_content;		
		$comment .= '<a href="?admin&apps=post&go=comment&act=del&id='.$row_comment->comment_id.'" class="button button4 right red" onclick="return confirm(\'Are You sure delete this post?\')">Hapus</a></div>';
...
if(!function_exists('save_post')){
	function save_post($data){
		...
		$data = compact('user_login','title','sefttitle','post_topic','mail','type','content','thumb','tags','date_post','meta_keys','meta_desc');		
